...
int main(int argc,char *argv[])
{
    ...
    msg_to_server=(Comm_struct *)shared_memory;

    //挂载本地内存
    //生成key  
    key_t client_key = ftok(".",getpid());  
    //创建共享内存段  
    client_shmid = shmget(client_key,12,IPC_CREAT|IPC_EXCL|0600);  
    if(client_shmid<0)perror("error"),exit(-1);  
    
    //挂接共享内存段  
    void *client_shared_memory = shmat(client_shmid,NULL,0);  
    if(client_shared_memory==(void*)-1)perror("error"),exit(-1); 
    msg_from_server= (Comm_struct *)client_shared_memory;


    msg_to_server->shm_id=client_shmid;
