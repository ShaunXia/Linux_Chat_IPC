...
int main(int argc,char *argv[])
{
    ...
    if(!strcmp("start",argv[1]))
    {
        ...
        Comm_struct *msg_from_client_shm,*msg_to_client_shm;
        ...
        if(shared_memory==(void*)-1)perror("error"),exit(-1); 
        msg_from_client_shm=(Comm_struct *)shared_memory;
        ...
        while(_running)
        {
            ...
            while(msg_from_client_shm->getNew)
            { 
                int apid;

                for(int j=0;j<user_count;++j)
                {
                    if(!strcmp(msg_from_client_shm->destination,user[j]))
                        apid=user_pid[j];
                }
                key_t key = ftok(".",apid);  
                //获取共享内存段  
                int  client_shmid = shmget(key,0,0);  
                void *client_shared_memory = shmat(client_shmid,NULL,0);  
                if(client_shared_memory==(void*)-1)perror("error"),exit(-1); 
                msg_to_client_shm=(Comm_struct *)client_shared_memory;
                sprintf(msg_to_client_shm->original,"%s",msg_from_client_shm->original);
                sprintf(msg_to_client_shm->message,"%s",msg_from_client_shm->message);
                msg_to_client_shm->getNew=1;
                msg_from_client_shm->getNew=0;
                if(shmdt(client_shared_memory)<0)perror("error"),exit(0);  
